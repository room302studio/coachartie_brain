  ...
  <div class="font-mono">
    ...
    <div class="flex items-center justify-between mb-1 border-b border-gray-300 dark:border-primary pb-0.5">
      <div class="flex items-center">
        ...
        <span class="text-[10px] text-gray-700 dark:text-tertiary">[{{ filteredMessages.length }}]</span>
      </div>
      <div>
        <button 
          @click="messagesToShow = 10"
          class="text-[10px] text-gray-700 dark:text-tertiary font-mono px-0.5"
          :class="messagesToShow === 10 ? 'border-b border-gray-700 dark:border-primary text-gray-900 dark:text-secondary' : ''"
        >
          10
        </button>
        <button 
          ...
          class="text-[10px] text-gray-700 dark:text-tertiary font-mono px-0.5"
          :class="messagesToShow === 25 ? 'border-b border-gray-700 dark:border-primary text-gray-900 dark:text-secondary' : ''"
        >
          25
        </button>
        <button 
          @click="messagesToShow = 50"
          class="text-[10px] text-gray-700 dark:text-tertiary font-mono px-0.5"
          :class="messagesToShow === 50 ? 'border-b border-gray-700 dark:border-primary text-gray-900 dark:text-secondary' : ''"
        >
          50
        </button>
      </div>
    ...
    <div class="space-y-0.5 max-h-[calc(100vh-250px)] overflow-y-auto">
      ...
      >
        <div class="flex items-center justify-between border-b border-gray-300 dark:border-secondary p-0.5">
          <div class="flex items-center">
            <span class="text-[10px] text-gray-900 dark:text-primary font-medium">[{{ message.user_id || 'SYSTEM' }}]</span>
            <span v-if="message.role" class="text-[10px] text-gray-700 dark:text-tertiary ml-1">({{ message.role }})</span>
          </div>
        ...
        <div class="p-0.5">
          <pre class="text-[10px] text-gray-900 dark:text-primary border-b border-gray-300 dark:border-secondary p-0.5 whitespace-pre-wrap break-words">{{ message.value }}</pre>
      ...
      <!-- Empty state -->
      <div v-if="filteredMessages.length === 0" class="text-center py-2 border-b border-dashed border-gray-300 dark:border-secondary">
        <span class="text-[10px] text-gray-700 dark:text-tertiary">NO_MESSAGES_FOUND</span>
      </div>
    </div>
  </div>
</template>
